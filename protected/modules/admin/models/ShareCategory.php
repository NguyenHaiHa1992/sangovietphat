 ...
 */
class ShareCategory extends CActiveRecord
{
	 ...
	 * @return shareCategory the static model class
	 */
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'shareCategory','parent_id')
		);
	}
	...
	*/
		...
		if($this->isNewRecord)
		{
			...
			foreach ( $list_order_view as $id => $order ) {
				if ($id != $this->id && $order >= $this->order_view) {
					$category = shareCategory::model ()->findByPk ( $id );
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = shareCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=shareCategory::model()->findAll('parent_id = '.$id);
		if(sizeof($list_category)>0){
			return self::DELETE_HAS_CHILD;
		}
		$list=shareCategory::model()->findAll('cat_id = '. $id);
		if(sizeof($list)>0) 
