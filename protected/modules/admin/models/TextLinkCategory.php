 ...
 */
class TextLinkCategory extends CActiveRecord
{
	 ...
	 * @return TextLinkCategory the static model class
	 */
	 ...
	 * Config scope of textLinkCategory
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'TextLinkCategory','parent_id')
		);
	}
	...
	*/
		...
		if($this->isNewRecord)
		{
			...
			foreach ( $list_order_view as $id => $order ) {
				if ($id != $this->id && $order >= $this->order_view) {
					$category = TextLinkCategory::model ()->findByPk ( $id );
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = TextLinkCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=TextLinkCategory::model()->findAll('parent_id = '.$id);
		if(sizeof($list_category)>0){
			return self::DELETE_HAS_CHILD;
		}
		$list=TextLinkCategory::model()->findAll('cat_id = '. $id);
		if(sizeof($list)>0) 
