 ...
 */
class IntroCategory extends CActiveRecord
{
	 ...
	 * @return IntroCategory the static model class
	 */
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'IntroCategory','parent_id')
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'status' => iPhoenixLang::admin_t('Status','intro'),
			'name' => iPhoenixLang::admin_t('Category','intro'),
			'parent_id' => iPhoenixLang::admin_t('Parent category','intro'),
			'order_view' => iPhoenixLang::admin_t('Order view','intro'),
	...
	*/
		...
		if($this->isNewRecord)
		{
			...
			foreach ( $list_order_view as $id => $order ) {
				if ($id != $this->id && $order >= $this->order_view) {
					$category = IntroCategory::model ()->findByPk ( $id );
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = IntroCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=IntroCategory::model()->findAll('parent_id = '.$id);
		if(sizeof($list_category)>0){
			return self::DELETE_HAS_CHILD;
		}
		$list=Intro::model()->findAll('cat_id = '. $id);
		if(sizeof($list)>0) 
