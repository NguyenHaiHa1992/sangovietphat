...
class NewsCategory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('name,alias,meta_keyword,meta_description,meta_title', 'length', 'max'=>256),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'status' => iPhoenixLang::admin_t('Status','news'),
			'name' => iPhoenixLang::admin_t('Category','news'),
			'parent_id' => iPhoenixLang::admin_t('Parent category','news'),
			'order_view' => iPhoenixLang::admin_t('Order view','news'),
			'meta_title' => 'Meta Title',
			'meta_keyword' => 'Meta Keyword',
			'meta_description' => 'Meta Description',
			'alias'=>iPhoenixLang::admin_t('Alias','news')
		);
	}
	 ...
	 */
	public static function getList_active_nodes(){
	...
	*/
	...
	public function updateOrderView() {
		if($this->isNewRecord)
		{
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = NewsCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=NewsCategory::model()->findAll('parent_id = '.$id);
		if(sizeof($list_category)>0){
			return self::DELETE_HAS_CHILD;
		}
		$list=News::model()->findAll('cat_id = '. $id);
		if(sizeof($list)>0) 
	 ...
	 */
	 public function getDetail_url(){
		return iPhoenixUrl::createUrl('news/list',array('cat_id'=>$this->id,'cat_alias'=>$this->alias));
	 }

	/*
	 * Get latest news
	 */
	 public function getLatest_news(){
		$criteria = new CDbcriteria();
