...
class AlbumCategory extends CActiveRecord
{
	...
	const TYPE_VIEW_ICON=1;
	const TYPE_VIEW_WALLPAPER=2;
	static public $config_type_view=array(
		self::TYPE_VIEW_ICON=>'Icon',
		self::TYPE_VIEW_WALLPAPER=>'Wallpaper'
	);
	
	private $config_other_attributes=array('meta_keyword','meta_description','meta_title','introimage_id','type_view');	
	...
	public function rules()
	{
		...
		return array(
			array('status,name, parent_id, order_view,meta_keyword,meta_description,meta_title, type_view', 'required'),
			array('parent_id','validatorParent'),
			array('order_view','validatorOrder_view'),
			array('status, parent_id, order_view, introimage_id, type_view', 'numerical', 'integerOnly'=>true),
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'AlbumCategory','parent_id'),
	...
	*/
		...
		if($this->isNewRecord)
		{
			...
			foreach ( $list_order_view as $id => $order ) {
				if ($id != $this->id && $order >= $this->order_view) {
					$category = AlbumCategory::model ()->findByPk ( $id );
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = AlbumCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=AlbumCategory::model()->findAll('parent_id = '.$id);
		if(sizeof($list_category)>0){
			return self::DELETE_HAS_CHILD;
		}
		$list=Album::model()->findAll('cat_id = '. $id);
		if(sizeof($list)>0) 
	 ...
	 */
	 public function getDetail_url(){
	 	return iPhoenixUrl::createUrl('album/list',array('cat_id'=>$this->id,'album_category_alias'=>iPhoenixString::createAlias($this->name)));
	 }
	 /*
	  * Get size image
	  */
	  public function getWidthThumb(){
	  	switch ($this->type_view){
	  		case self::TYPE_VIEW_ICON:
				return 90;
				break;
			case self::TYPE_VIEW_WALLPAPER:
				return 320;	
				break;
	  	}
	  }
