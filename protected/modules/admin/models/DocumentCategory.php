...
class DocumentCategory extends CActiveRecord
{
	...
	private $config_other_attributes=array('meta_keyword','meta_description','meta_title','introimage_id');	
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'DocumentCategory','parent_id')
		);
	}
	...
	*/
	...
	public function updateOrderView() {
		if($this->isNewRecord)
		{
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = DocumentCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=DocumentCategory::model()->findAll('parent_id = '.$id);
	 ...
	 */
	 public function getDetail_url(){
		return iPhoenixUrl::createUrl('document/list',array('id'=>$this->id,'cat_alias'=>iPhoenixString::createAlias($this->name)));
	 }
}