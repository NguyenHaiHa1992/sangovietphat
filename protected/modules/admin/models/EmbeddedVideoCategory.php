 ...
 */
class EmbeddedVideoCategory extends CActiveRecord
{
	 ...
	 * @return EmbeddedVideoCategory the static model class
	 */
	 ...
	 * Config scope of embeddedVideoCategory
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'EmbeddedVideoCategory','parent_id')
		);
	}
	...
	*/
		...
		if($this->isNewRecord)
		{
			...
			foreach ( $list_order_view as $id => $order ) {
				if ($id != $this->id && $order >= $this->order_view) {
					$category = EmbeddedVideoCategory::model ()->findByPk ( $id );
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = EmbeddedVideoCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=EmbeddedVideoCategory::model()->findAll('parent_id = '.$id);
		if(sizeof($list_category)>0){
			return self::DELETE_HAS_CHILD;
		}
		$list=EmbeddedVideoCategory::model()->findAll('cat_id = '. $id);
		if(sizeof($list)>0) 
	 ...
	 */
	 public function getDetail_url(){
	 	return Yii::app()->createUrl('embeddedVideo/list',array('id'=>$this->id,'category_alias'=>iPhoenixString::createAlias($this->name)));
	 }
}