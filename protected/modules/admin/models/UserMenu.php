...
class UserMenu extends CActiveRecord
{
	...
	private $config_other_attributes=array('url','title','target','rel');	
	...
	public function rules()
	{
		...
		return array(
			array('status, name, parent_id, url', 'required'),
			...
			array('name, url, target, rel, title', 'length', 'max'=>256),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'UserMenu','parent_id'),
		);
	}
	...
	*/
	...
	public function findActiveMenu(){
		...
		foreach ($list as $id=>$level){
			$menu=self::model()->findByPk($id);
			$short_url = str_replace($host, '', $menu->url);
			$short_url = preg_replace('{/$}', '', $short_url);

			if(in_array($short_url,$current_url))
			{
	...
	static function getListItem(){
		...
		if(sizeof($list_nodes)>0){
			$first=true;
			foreach ($list_nodes as $id=>$level) {
				$list[$id]['content'] = UserMenu::model()->findByPk($id);
				$list[$id]['level']=$level;
				if($first==true) {
					$list[$id]['first']=true;
					$first=false;
