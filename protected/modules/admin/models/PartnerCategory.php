 ...
 */
class PartnerCategory extends CActiveRecord
{
	 ...
	 * @return PartnerCategory the static model class
	 */
	 ...
	 * Config scope of partnerCategory
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO,'PartnerCategory','parent_id')
		);
	}
	...
	*/
		...
		if($this->isNewRecord)
		{
			...
			foreach ( $list_order_view as $id => $order ) {
				if ($id != $this->id && $order >= $this->order_view) {
					$category = PartnerCategory::model ()->findByPk ( $id );
			...
			if ($this->parent_id == $this->old_parent_id) {
				...
				$list = PartnerCategory::model ()->findAll ( 'parent_id=' . $this->old_parent_id );
	...
	public function checkDelete($id){
		$list_category=PartnerCategory::model()->findAll('parent_id = '.$id);
		if(sizeof($list_category)>0){
			return self::DELETE_HAS_CHILD;
		}
		$list=PartnerCategory::model()->findAll('cat_id = '. $id);
		if(sizeof($list)>0) 
