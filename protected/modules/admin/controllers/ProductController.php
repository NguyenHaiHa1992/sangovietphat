...
class ProductController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow',
                'actions' => array('edit', 'reverseBestSell'),
                'roles' => array('product_edit'),
            ),
            array('allow', 
                'actions' => array('bestSell'),
                'roles' => '@'
            ),
            array('deny',
                'users' => array('*'),
    ...
    public function actionReverseBestSell($id) {
        $product = Product::model()->findByPk($id);
        $old_status = $product->best_sell;
        $product->best_sell = !$old_status;
        if ($product->save())
            echo json_encode(array('success' => true, 'src' => $product->getAttributeImageStatus('best_sell')));
        else
            echo json_encode(array('success' => false));
    }
