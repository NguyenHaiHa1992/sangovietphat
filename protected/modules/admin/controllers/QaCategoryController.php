...
/**
 * 
 * QaCategoryController class file 
...
/**
 * QaCategoryController includes actions relevant to QaCategory:
 *** create new QaCategory
 *** update information of a QaCategory
 *** delete QaCategory
 *** validate category
 *** index category
 *** write 
...
class QaCategoryController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  
				...
				'roles'=>array('qaCategory_create'),
			),
			array('allow', 
				'actions'=>array('update'),
				'roles'=>array('qaCategory_update'),
			),
			array('allow',  
				'actions'=>array('delete'),
				'roles'=>array('qaCategory_delete'),
			),
			array('allow',  
				'actions'=>array('validate'),
				'roles'=>array('qaCategory_validate'),
			),
			array('allow',  
				'actions'=>array('index'),
				'roles'=>array('qaCategory_index'),
			),
			array('allow',  
				'actions'=>array('write'),
				'roles'=>array('qaCategory_write'),
			),
			array('allow',  
				'actions'=>array('updateListOrderView'),
				'roles'=>array('qaCategory_updateListOrderView'),
			),
			array('deny', 
				'users'=>array('*'),
			),
	...
	public function actionDelete($id,$current_id)
	{
			...
			switch ((int)$model->checkDelete($id))	{
				case QaCategory::DELETE_OK:		
					...
					break;
				case QaCategory::DELETE_HAS_CHILD:
					$result['status']= false;
					$result['content'] = "Bạn phải xóa hết các thư mục con.";
					break;
				case QaCategory::DELETE_HAS_ITEMS:
					$result['status']= false;
	...
	public function actionValidate()
	{
		...
		{
			if($_POST['id']>0){
				$model=QaCategory::model()->findByPk($_POST['id']);
			}
			else {
				$model=new QaCategory();
			}
	...
	/**
	 * Creates and updates a new QaCategory model.
	 * @return 
	 */
	public function actionWrite()
	{	
		if(isset($_GET['QaCategory']))
		{
			$id=(int)$_GET['id'];
			if ( is_int($id) && $id>0){
				$action="update";
			...
			$model->attributes=$_GET['QaCategory'];	
			if($action == 'create'){
				$list=QaCategory::model()->findAll('parent_id='.$model->parent_id);
	...
	{	
		$list=QaCategory::model()->findAll('parent_id='.$parent_id);
	...
	{
		$model=QaCategory::model()->findByPk($id);
		if($model===null)
