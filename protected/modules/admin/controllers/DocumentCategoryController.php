...
/**
 * 
 * DocumentCategoryController class file 
...
/**
 * DocumentCategoryController includes actions relevant to DocumentCategory:
 *** create new DocumentCategory
 *** update information of a DocumentCategory
...
class DocumentCategoryController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  
				...
				'roles'=>array('documentCategory_create'),
			),
			array('allow', 
				'actions'=>array('update'),
				'roles'=>array('documentCategory_update'),
			),
			array('allow',  
				'actions'=>array('delete'),
				'roles'=>array('documentCategory_delete'),
			),
			array('allow',  
				'actions'=>array('validate'),
				'roles'=>array('documentCategory_validate'),
			),
			array('allow',  
				'actions'=>array('index'),
				'roles'=>array('documentCategory_index'),
			),
			array('allow',  
				'actions'=>array('write'),
				'roles'=>array('documentCategory_write'),
			),
			array('allow',  
				'actions'=>array('updateListOrderView'),
				'roles'=>array('documentCategory_updateListOrderView'),
			),
			array('deny', 
				'users'=>array('*'),
			),
	...
	public function actionDelete($id,$current_id)
	{
			...
			switch ((int)$model->checkDelete($id))	{
				case DocumentCategory::DELETE_OK:		
					if($model->delete()) {
						$result['status']=true;
					...
					break;
				case DocumentCategory::DELETE_HAS_CHILD:
					$result['status']= false;
					$result['content'] = "Có thư mục con.";
					break;
				case DocumentCategory::DELETE_HAS_ITEMS:
					$result['status']= false;
	...
	public function actionValidate()
	{
		...
		{
			if($_POST['id']>0){
				$model=DocumentCategory::model()->findByPk($_POST['id']);
			}
			else {
				$model=new DocumentCategory();
			}
	...
	/**
	 * Creates and updates a new DocumentCategory model.
	 * @return 
	 */
	...
	{	
		if(isset($_GET['DocumentCategory']))
		{
			...
			if($action == 'create'){
				$list=DocumentCategory::model()->findAll('parent_id='.$model->parent_id);
	...
	{	
		$list=DocumentCategory::model()->findAll('parent_id='.$parent_id);
