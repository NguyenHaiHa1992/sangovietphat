...
/**
 * 
 * VideoCategoryController class file 
...
/**
 * VideoCategoryController includes actions relevant to VideoCategory:
 *** create new VideoCategory
 *** update information of a VideoCategory
...
class VideoCategoryController extends Controller
{
	...
	public function accessRules()
	{
		...
		array('allow',  
				...
				'roles'=>array('videoCategory_create'),
			),
			array('allow', 
				'actions'=>array('update'),
				'roles'=>array('videoCategory_update'),
			),
			array('allow',  
				'actions'=>array('delete'),
				'roles'=>array('videoCategory_delete'),
			),
			array('allow',  
				'actions'=>array('validate'),
				'roles'=>array('videoCategory_validate'),
			),
			array('allow',  
				'actions'=>array('index'),
				'roles'=>array('videoCategory_index'),
			),
			array('allow',  
				'actions'=>array('write'),
				'roles'=>array('videoCategory_write'),
			),
			array('allow',  
				'actions'=>array('updateListOrderView'),
				'roles'=>array('videoCategory_updateListOrderView'),
			),
			array('deny', 
				'users'=>array('*'),
			),
	...
	public function actionValidate()
	{
		...
		{
			if($_POST['id']>0){
				$model=VideoCategory::model()->findByPk($_POST['id']);
			}
			else {
				$model=new VideoCategory();
			}
	...
	/**
	 * Creates and updates a new VideoCategory model.
	 * @return 
	 */
	...
	{	
		if(isset($_GET['VideoCategory']))
		{
			...
			if($action == 'create'){
				$list=VideoCategory::model()->findAll('parent_id='.$model->parent_id);
	...
	{	
		$list=VideoCategory::model()->findAll('parent_id='.$parent_id);
