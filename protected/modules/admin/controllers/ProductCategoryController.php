...
/**
 * 
 * ProductCategoryController class file 
...
/**
 * ProductCategoryController includes actions relevant to ProductCategory:
 *** create new ProductCategory
 *** update information of a ProductCategory
...
class ProductCategoryController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  
				...
				'roles'=>array('productCategory_create'),
			),
			array('allow', 
				'actions'=>array('update'),
				'roles'=>array('productCategory_update'),
			),
			array('allow', 
				'actions'=>array('detailUpdate'),
				'roles'=>array('productCategory_update'),
			),
			array('allow',  
				'actions'=>array('delete'),
				'roles'=>array('productCategory_delete'),
			),
			array('allow',  
				'actions'=>array('validate'),
				'roles'=>array('productCategory_validate'),
			),
			array('allow',  
				'actions'=>array('index'),
				'roles'=>array('productCategory_index'),
			),
			array('allow',  
				'actions'=>array('write'),
				'roles'=>array('productCategory_write'),
			),
			array('allow',  
				'actions'=>array('updateListOrderView'),
				'roles'=>array('productCategory_updateListOrderView'),
			),
			array('deny', 
				'users'=>array('*'),
			),
	...
	public function actionDelete($id,$current_id)
	{
			...
			switch ((int)$model->checkDelete($id))	{
				case ProductCategory::DELETE_OK:		
					if($model->delete()) {
						$result['status']=true;
					...
					break;
				case ProductCategory::DELETE_HAS_CHILD:
					$result['status']= false;
					$result['content'] = iPhoenixLang::admin_t('You must delete all sub-category','main');
					break;
				case ProductCategory::DELETE_HAS_ITEMS:
					$result['status']= false;
	...
	public function actionValidate()
	{
		...
		{
			if($_POST['id']>0){
				$model=ProductCategory::model()->findByPk($_POST['id']);
			}
			else {
				$model=new ProductCategory();
			}
	...
	/**
	 * Creates and updates a new ProductCategory model.
	 * @return 
	 */
	...
	{	
		if(isset($_POST['ProductCategory']))
		{
			...
			if($action == 'create'){
				$list=ProductCategory::model()->findAll('parent_id='.$model->parent_id);
	...
	{	
		$list=ProductCategory::model()->findAll('parent_id='.$parent_id);
