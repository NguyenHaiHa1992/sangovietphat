...
/**
 * 
 * AudioCategoryController class file 
...
/**
 * AudioCategoryController includes actions relevant to AudioCategory:
 *** create new AudioCategory
 *** update information of a AudioCategory
 *** delete AudioCategory
 *** validate category
 *** index category
 *** write 
...
class AudioCategoryController extends Controller
{
	...
	public function accessRules()
	{
		...
		array('allow',  
				...
				'roles'=>array('audioCategory_create'),
			),
			array('allow', 
				'actions'=>array('update'),
				'roles'=>array('audioCategory_update'),
			),
			array('allow',  
				'actions'=>array('delete'),
				'roles'=>array('audioCategory_delete'),
			),
			array('allow',  
				'actions'=>array('validate'),
				'roles'=>array('audioCategory_validate'),
			),
			array('allow',  
				'actions'=>array('index'),
				'roles'=>array('audioCategory_index'),
			),
			array('allow',  
				'actions'=>array('write'),
				'roles'=>array('audioCategory_write'),
			),
			array('allow',  
				'actions'=>array('updateListOrderView'),
				'roles'=>array('audioCategory_updateListOrderView'),
			),
			array('deny', 
				'users'=>array('*'),
			),
	...
	public function actionValidate()
	{
		...
		{
			if($_POST['id']>0){
				$model=AudioCategory::model()->findByPk($_POST['id']);
			}
			else {
				$model=new AudioCategory();
			}
	...
	/**
	 * Creates and updates a new AudioCategory model.
	 * @return 
	 */
	...
	{	
		if(isset($_GET['AudioCategory']))
		{
			...
			if($action == 'create'){
				$list=AudioCategory::model()->findAll('parent_id='.$model->parent_id);
	...
	{	
		$list=AudioCategory::model()->findAll('parent_id='.$parent_id);
