<?php $form=$this->beginWidget('CActiveForm', array('id'=>'category-form','enableAjaxValidation'=>true,'clientOptions'=>array('validationUrl'=>$this->createUrl('advisorCategory/validate')))); ?>	
...
<div class="fl" style="width:500px;">
	<ul>
		...
		<div class="row">
		  ...
		  <div class="row" <?php if(AdvisorCategory::MAX_RANK == 1) echo'style="display:none"'?>>
		<li>
			<?php echo $form->labelEx($model,'parent_id'); ?>
			<?php
				$view_parent_nodes=array('0'=>'Original');
				foreach ($model->list_nodes as $id=>$level){
					$node=AdvisorCategory::model()->findByPk($id);
					$view = "";
					for($i=1;$i<$level;$i++){
						$view .="--";
					}
		...
		<li>
			...
			<?php
				$list=array(AdvisorCategory::STATUS_ENABLE=>'Show',AdvisorCategory::STATUS_DISABLE=>'Hide');
		...
		<li>
			...
			if($action=="update") 
			{   
				echo '<input type="submit" value="Add new category" style="margin-left:10px; width:125px;" id="create-category" class="button">'; 
			}
			?>  
		</li>
	</ul>
