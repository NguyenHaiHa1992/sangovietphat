...
<div id="main-content" class="folder top">
	...
	<div class="folder-header">
		<h1>Danh mục Chia Sẻ</h1>
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon" href=""><span>Danh mục Chia Sẻ</span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
  			...
  			jConfirm(\"Bạn muốn xóa danh mục này?\",\"Notice\",function(r){
	  			if(r==true){
					jQuery.ajax({
						...
						'type':'GET',
						'url':'".iPhoenixUrl::createUrl('admin/shareCategory/delete')."',
						'cache':false});
	        	};
  			});
        	return false;	
		...
		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
					...
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/shareCategory/update')."',
					'cache':false
				});
				return false;
			}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/shareCategory/create')."',
        			'cache':false,
        		});
        		return false;
        	}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/shareCategory/write')."',
        			'cache':false,
...
$cs->registerScript(
  ...
  "jQuery(
	...
	{ 
		$('body').on(
  			'change',
  			'#shareCategory_parent_id',	
  			function(){
  				jQuery.ajax({
  					'data':{parent_id : $(this).val()},
  					'success':function(data){  		
  						if($(\"#shareCategory_order_view\")){				
							$(\"#shareCategory_order_view\").html(\"\");
							for(var i=1;i<=data;i++){
								...
								$(\"#shareCategory_order_view\").append(option);
							}
						}
					},
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/shareCategory/updateListOrderView')."',
					'cache':false
				});
				return false;
			}
