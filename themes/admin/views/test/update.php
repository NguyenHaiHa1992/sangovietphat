...
<div class="folder top">
	...
	<div class="folder-header">
		<h1><?php iPhoenixLang::admin_t('Testing management');?></h1>
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon"><span><?php iPhoenixLang::admin_t('Update Testing');?></span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
	<div class="folder-content form">
		<div>
            <input type="button" class="button" value="<?php iPhoenixLang::admin_t('List Testings');?>" style="width:180px;" onClick="parent.location='<?php echo iPhoenixUrl::createUrl('admin/test')?>'"/>
            <input type="button" class="button" value="<?php iPhoenixLang::admin_t('Add Testing');?>" style="width:180px;" onClick="parent.location='<?php echo iPhoenixUrl::createUrl('admin/test/create')?>'"/>
			...
			<ul>
				...
				<div class="row">
					<li>
						...
						<a title="<?php iPhoenixLang::admin_t('Choose Testing');?>" onclick="showPopUp('list_suggest_test');return false;" id="btn-add-product" class="button" style="width: 100px;text-decoration: none;"><?php echo iPhoenixLang::admin_t('Choose','main');?></a>			
					</li>
				</div>	
				<div class="row">
					<li>
...
<?php	echo $this->renderPartial('_list_suggest_test', 
								array(
									'class_checkbox'=>'SuggestTest',
									'class'=>'list_suggest_test',
...
setInterval (
     function () {
        ...
        if (status != "on" && autosave == "on") // if no function is running, then run the function
        {
           ...
           $.ajax ({
              url: "<?php echo Yii :: app () -> createUrl ('admin/test/autoSave',array('id'=>$model->id))?>", // ​​call a function to autosave
              type: "post",
              dataType: "json",
...
/*** Preview test before saving it script ***/
$('#btn-preview').click(function(){
   $.ajax ({
      url: "<?php echo Yii::app()->createUrl('test/preview')?>", // ​​call a function to preview
      type: "post",
	  dataType: "json",
      data: $('form',$('.folder-content')).serialize(),
      success: function (data) {
