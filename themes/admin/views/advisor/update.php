...
<div  id="main-content" class="folder top">
	...
	<div class="folder-header">
		...
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon" href=""><span>Edit Advisor</span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
	<div class="folder-content form">
		<div>
            <input type="button" class="button" value="List Advisors" style="width:180px;" onClick="parent.location='<?php echo iPhoenixUrl::createUrl('admin/advisor')?>'"/>
			...
			<ul>
				...
				<div class="row none">
					<li>
						...
						<a title="Chọn Cố vấn" onclick="showPopUp('list_suggest_advisor');return false;" id="btn-add-product" class="button" style="width: 100px;text-decoration: none;">Chọn</a>			
					</li>
				</div>	
				<div class="row">
					<li>
...
<?php	echo $this->renderPartial('_list_suggest_advisor', 
								array(
									'class_checkbox'=>'SuggestAdvisor',
									'class'=>'list_suggest_advisor',
...
setInterval (
     function () {
        ...
        if (status != "on" && autosave == "on") // if no function is running, then run the function
        {
           ...
           $.ajax ({
              url: "<?php echo Yii :: app () -> createUrl ('admin/advisor/autoSave',array('id'=>$model->id))?>", // ​​call a function to autosave
              type: "post",
              dataType: "json",
...
$('#Advisor_title').blur(function(){
	var name = $(this).val();
	$('#Advisor_alias').val(createAlias(name));
});
