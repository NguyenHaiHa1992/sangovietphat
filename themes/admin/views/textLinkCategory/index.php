...
<div id="main-content" class="folder top">
	...
	<div class="folder-header">
		<h1>Text link categories</h1>
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon" href=""><span>Text link categories</span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
  			...
  			jConfirm(\"Are you sure you want to delete this category?\",\"Notice\",function(r){
	  			if(r==true){
					jQuery.ajax({
						...
						'type':'GET',
						'url':'".iPhoenixUrl::createUrl('admin/textLinkCategory/delete')."',
						'cache':false});
	        	};
  			});
        	return false;	
		...
		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
					...
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/textLinkCategory/update')."',
					'cache':false
				});
				return false;
			}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/textLinkCategory/create')."',
        			'cache':false,
        		});
        		return false;
        	}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/textLinkCategory/write')."',
        			'cache':false,
...
$cs->registerScript(
  ...
  "jQuery(
	...
	{ 
		$('body').on(
  			'change',
  			'#TextLinkCategory_parent_id',	
  			function(){
  				jQuery.ajax({
  					'data':{parent_id : $(this).val()},
  					'success':function(data){  		
  						if($(\"#TextLinkCategory_order_view\")){				
							$(\"#TextLinkCategory_order_view\").html(\"\");
							for(var i=1;i<=data;i++){
								...
								$(\"#TextLinkCategory_order_view\").append(option);
							}
						}
					},
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/textLinkCategory/updateListOrderView')."',
					'cache':false
				});
				return false;
			}
