<?php $form = $this -> beginWidget('CActiveForm', array('id' => 'category-form', 'enableAjaxValidation' => true, 'clientOptions' => array('validationUrl' => $this -> createUrl('productCategory/validate')))); ?>
...
<div class="fl" style="width:500px; overflow: visible">
	<ul>
        ...
        <div class="row">
            <li>
                <?php echo $form -> labelEx($model, 'boximage_id'); ?>
                <?php echo $this -> renderPartial('/image/_signupload', array('model' => $model, 'attribute' => 'boximage_id', 'h' => 50, 'w' => 150)); ?>
                <?php echo $form -> error($model, 'boximage_id'); ?>
            </li>
        </div>

        <div class="row">
            <li>
                <?php echo $form -> labelEx($model, 'catimage_id'); ?>
                <?php echo $this -> renderPartial('/image/_signupload', array('model' => $model, 'attribute' => 'catimage_id', 'h' => 50, 'w' => 150)); ?>
                <?php echo $form -> error($model, 'catimage_id'); ?>
            </li>
        </div>

		<div class="row" <?php if(ProductCategory::MAX_RANK == 1) echo'style="display:none"'?>>
			<li>
				<?php echo $form -> labelEx($model, 'parent_id'); ?>
				<?php $view_parent_nodes = array('0' => iPhoenixLang::admin_t('Root','product'));
				foreach ($model->list_nodes as $id => $level) {
					$node = ProductCategory::model() -> findByPk($id);
					$view = "";
					for ($i = 1; $i < $level; $i++) {
						$view .= "--";
					}
		...
		<div class="row">
			<li>
				...
				<?php $list = array(ProductCategory::STATUS_ENABLE => iPhoenixLang::admin_t('Show','main'), ProductCategory::STATUS_DISABLE => iPhoenixLang::admin_t('Hide','main'));
		...
		<div class="row">
			<div id="tabContainer">
				<div id="tabMenu">
					<ul class="menu">
						<li><a id="select1" class="active"><span><?php echo $form -> labelEx($model, 'description'); ?></span></a></li>
						<li><a id="select2"><span><?php echo $form -> labelEx($model, 'extra'); ?></span></a></li>
					</ul>
				</div>
		...
		<li>
			...
			if ($action == "update") {
				echo '<input type="submit" value="Add product categories" style="margin-left:10px; width:125px;" id="create-category" class="button">';
			}
			?>
		</li>
	</ul>
...
$(document).ready(function(){
	tinyMCE.init({
		mode : "exact",
		elements : "ProductCategory_description, ProductCategory_extra",
		theme : "advanced",
		skin : "bootstrap",
		width: "800",
		height: "500",
