...
<div id="main-content" class="folder top">
	...
	<div class="folder-header">
		...
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon" href=""><span>Add Text link</span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
	...
	<div>
		...
		<input type="button" class="button" value="List Text links" style="width:180px;" onClick="parent.location='<?php echo iPhoenixUrl::createUrl('admin/textlink')?>'"/>
		...
		<div>
			<ul>
                ...
                <li>
					...
					<input type="button" class="button" id="btn-preview" value="Xem trước" style="margin-left:20px; width:125px;" />
...
setInterval (
     function () {
        ...
        if (status != "on" && autosave == "on") // if no function is running, then run the function
        {
           ...
           $.ajax ({
              url: "<?php echo Yii :: app () -> createUrl ('admin/textlink/autoSave')?>", // ​​call a function to autosave
              type: "post",
              dataType: "json",
   ...
   $.ajax ({
      ...
      success: function (data) {
            if (data.success) {
				...
				jAlert('Error, please fill all required information Text link!');
            }
      },
  });
});
