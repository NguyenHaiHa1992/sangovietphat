...
<div id="main-content" class="folder top">
	...
	<div class="folder-header">
		<h1>Album Categories</h1>
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon" href=""><span>Album Categories</span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
...
$cs->registerScript(
  ...
  "jQuery(function($) { $('body').on('click','.i16-trashgray',	
  		function(){
  			...
  			jConfirm(\"Are you sure you want to delete this category?\",\"Confirm delete\",function(r){
	  			if(r==true){
					jQuery.ajax({
						...
						'type':'GET',
						'url':'".iPhoenixUrl::createUrl('admin/albumCategory/delete')."',
						'cache':false});
	        	};
	        	});
        	return false;	
		...
		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
					...
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/albumCategory/update')."',
					'cache':false
				});
				return false;
			}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/albumCategory/create')."',
        			'cache':false,
        		});
        		return false;
        	}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/albumCategory/write')."',
        			'cache':false,
...
$cs->registerScript(
  ...
  "jQuery(
	...
	{ 
		$('body').on(
  			'change',
  			'#AlbumCategory_parent_id',	
  			function(){
  				jQuery.ajax({
  					'data':{parent_id : $(this).val()},
  					'success':function(data){  		
  						if($(\"#AlbumCategory_order_view\")){				
							$(\"#AlbumCategory_order_view\").html(\"\");
							for(var i=1;i<=data;i++){
								...
								$(\"#AlbumCategory_order_view\").append(option);
							}
						}
					},
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/albumCategory/updateListOrderView')."',
					'cache':false
				});
				return false;
			}
