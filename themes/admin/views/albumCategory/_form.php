<?php $form=$this->beginWidget('CActiveForm', array('id'=>'category-form','enableAjaxValidation'=>true,'clientOptions'=>array('validationUrl'=>$this->createUrl('albumCategory/validate')))); ?>	
<input type="hidden" name="id" id="current_id" value="<?php echo isset($model->id)?$model->id:'0';?>" /> 
			<div class="fl" style="width:500px;">
				<ul>
                    ...
                    <div class="row">
						<li>
                       		<?php echo $form->labelEx($model,'type_view'); ?>
                        	<?php echo $form->dropDownList($model,'type_view',AlbumCategory::$config_type_view,array('style'=>'width:200px')); ?>
                   			<?php echo $form->error($model, 'type_view'); ?>
                    	</li>
                    </div>  
                     <div class="row">
						<li>
							<?php echo $form->labelEx($model,'introimage_id'); ?>
							<?php echo $this->renderPartial('/image/_signupload', array('model'=>$model,'attribute'=>'introimage_id','h'=>100,'w'=>465)); ?>		
							<?php echo $form->error($model, 'introimage_id'); ?>
						</li>
					</div>	                                              
                     <div class="row" <?php if(AlbumCategory::MAX_RANK == 1) echo'style="display:none"'?>>
                    <li>
                        <?php echo $form->labelEx($model,'parent_id'); ?>
                        <?php
                        	$view_parent_nodes=array('0'=>'Gốc');
                        	foreach ($model->list_nodes as $id=>$level){
                        	 	$node=AlbumCategory::model()->findByPk($id);
								$view = "";
								for($i=1;$i<$level;$i++){
									$view .="--";
								}
                    ...
                    <li>
                        ...
                        <?php
                        	$list=array(AlbumCategory::STATUS_ENABLE=>'Show',AlbumCategory::STATUS_DISABLE=>'Hide');
