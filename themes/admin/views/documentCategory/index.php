...
<div id="main-content" class="folder top">
	...
	<div class="folder-header">
		...
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon" href=""><span>Document categories</span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
...
$cs->registerScript(
  ...
  "jQuery(function($) { $('body').on('click','.i16-trashgray',	
  		function(){
  			...
  			jConfirm(\"Are you sure you want to delete this category?\",\"Confirm deletion\",function(r){
	  			if(r==true){
					jQuery.ajax({
						...
						'type':'GET',
						'url':'".iPhoenixUrl::createUrl('admin/documentCategory/delete')."',
						'cache':false});
	        	};
	        	});
        	return false;	
		...
		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
					...
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/documentCategory/update')."',
					'cache':false
				});
				return false;
			}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/documentCategory/create')."',
        			'cache':false,
        		});
        		return false;
        	}
  		...
  		$('body').on(
  			...
  			function(){
  				jQuery.ajax({
        			...
        			'type':'GET',
        			'url':'".iPhoenixUrl::createUrl('admin/documentCategory/write')."',
        			'cache':false,
...
$cs->registerScript(
  ...
  "jQuery(
	...
	{ 
		$('body').on(
  			'change',
  			'#DocumentCategory_parent_id',	
  			function(){
  				jQuery.ajax({
  					'data':{parent_id : $(this).val()},
  					'success':function(data){  		
  						if($(\"#DocumentCategory_order_view\")){				
							$(\"#DocumentCategory_order_view\").html(\"\");
							for(var i=1;i<=data;i++){
								...
								$(\"#DocumentCategory_order_view\").append(option);
							}
						}
					},
					'type':'GET',
					'url':'".iPhoenixUrl::createUrl('admin/documentCategory/updateListOrderView')."',
					'cache':false
				});
				return false;
			}
