<?php $form=$this->beginWidget('CActiveForm', array('id'=>'category-form','enableAjaxValidation'=>true,'clientOptions'=>array('validationUrl'=>$this->createUrl('documentCategory/validate')))); ?>	
...
<div class="fl" style="width:500px;">
	<ul>
		...
		<div class="row">
         ...
         <div class="row" <?php if(DocumentCategory::MAX_RANK == 1) echo'style="display:none"'?>>
        <li>
            <?php echo $form->labelEx($model,'parent_id'); ?>
            <?php
            	$view_parent_nodes=array('0'=>'Gốc');
            	foreach ($model->list_nodes as $id=>$level){
            	 	$node=DocumentCategory::model()->findByPk($id);
					$view = "";
					for($i=1;$i<$level;$i++){
						$view .="--";
					}
        ...
        <li>
            ...
            <?php
            	$list=array(DocumentCategory::STATUS_ENABLE=>'Hiện',DocumentCategory::STATUS_DISABLE=>'Ẩn');
		...
		<div class="row">
       	...
       	<li>
        	...
        	if($action=="update") 
        	{ 
        		$label_button="Update document category";     
        	}
        	else $label_button="Add category";
        	
			echo '<input type="submit" value="'.$label_button.'" style="margin-left:153px; width:125px;" id="write-category" class="button">';  
			if($action=="update") 
        	{   
				echo '<input type="submit" value="Add category" style="margin-left:10px; width:125px;" id="create-category" class="button">'; 
        	}
			?>  
        </li>
	</ul>
