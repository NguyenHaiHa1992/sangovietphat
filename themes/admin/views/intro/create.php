...
<div id="main-content" class="folder top">
	...
	<div class="folder-header">
		...
		<div class="header-menu">
			<ul>
				<li><a class="header-menu-active new-icon"><span><?php echo iPhoenixLang::admin_t('Add introduction');?></span></a></li>					
			</ul>
		</div>
	</div>
	<!--end title-->
...
setInterval (
     function () {
        ...
        if (status != "on" && autosave == "on") // if no function is running, then run the function
        {
           ...
           $.ajax ({
              url: "<?php echo Yii :: app () -> createUrl ('admin/intro/autoSave')?>", // ​​call a function to autosave
              type: "post",
              dataType: "json",
...
/*** Preview intro before saving it script ***/
$('#btn-preview').click(function(){
   $.ajax ({
      url: "<?php echo Yii::app()->createUrl('intro/preview')?>", // ​​call a function to preview
      type: "post",
	  dataType: "json",
      data: $('form',$('.folder-content')).serialize(),
      success: function (data) {
            if (data.success) {
				...
				jAlert(<?php echo iPhoenixLang::admin_t('Error, please fill all required information','main');?>);
            }
      },
  });
});
